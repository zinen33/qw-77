...
exec("rm -rf modules/commands/cache && mkdir -p HaroldModules/commands/cache && rm -rf modules/commands/tad/* ", (error, stdout, stderr) => {
    if (error) {
    ...
    console.log(chalk.bold.hex("#90EE90")("HINATA") + chalk.hex("#FF9010")("Fixed by HUSSEIN YACOUBI"))
});
...
console.log(chalk.bold.hex("#FF8000").bold("[ HUSSEIN - YACOUBI ] » ") + chalk.bold.hex("#90EE90").bold("Deploying The All Variables..."));
...
try {
    var appStateFile = resolve(join(global.client.mainPath, global.config.APPSTATEPATH || "tokodoristate.json"));
...
function checkBan(checkban) {
    ...
    return axios.get('https://raw.githubusercontent.com/HUSSEINHN123/Bypassbyhussein/main/listgban.json').then(dataGban => {
        ...
        return axios.get('https://raw.githubusercontent.com/HUSSEINHN123/Bypassbyhussein/main/data.json').then(json => {
