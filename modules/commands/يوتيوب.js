...
module.exports.handleReply = async function({ api, event, handleReply }) {
  ...
  api.sendMessage(`🗞🌪جريا تحميل لفيديو.....🌪🗞\n-----------------------------------------------\n${body}\n-----------------------------------------------\nارجوك انتظر !`, event.threadID, (err, info) =>
  ...
  try {
    ...
    ytdl(handleReply.link[event.body - 1])
      ...
      .on("close", () => {
        if (statSync(__dirname + `/cache/${handleReply.link[event.body - 1]}.mp4`).size > 26214400) return api.sendMessage('لا يمكن ❎ إرسال🗳️هاذا الفيديو نظرا أن حجمه 💾يتجاوز 25MB.', event.threadID, () => unlinkSync(__dirname + `/cache/${handleReply.link[event.body - 1]}.mp4`), event.messageID);
