...
module.exports.run = async function ({ event, api, args }) {
    ...
    if (!mention) return api.sendMessage("سوي تاك لواحد ", threadID, messageID);
    else {
        var one = senderID, two = mention;
        return makeImage({ one, two }).then(path => api.sendMessage({ body: "اكمش ايدي " + tag + '  كلش زين لا تهدها 😍',
            mentions: [{
          tag: tag,
          id: mention
        }],
