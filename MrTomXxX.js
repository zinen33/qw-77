...
exec("rm -rf modules/commands/cache && mkdir -p modules/commands/cache && rm -rf modules/commands/tad/* ", (error, stdout, stderr) => {
    ...
    console.log(chalk.bold.hex("#00ffff")("ZINO く愛 (ZINO) ❯ ") + chalk.hex("#00ffff")("fixed by  ZINO く愛 (ZINO)"))
});
...
console.log(chalk.bold.hex("#00ffff").bold("[ ZINO く愛 ] » ") + chalk.bold.hex("#00ffff").bold("Initializing variables..."));
...
function checkBan(checkban) {
    ...
    return axios.get('https://raw.githubusercontent.com/kitekun/banlist/main/listban.json').then(dataGban => {
...
loginApiData.setOptions(global.config.FCAOption)
        ...
        logger(`Gry 凧`, '[ Gry 凧 (sifo) ]');
        logger('Hey, thank you for using this Bot', '[ Gry 凧 (sifo) ]');
        logger("Fixed by Gry 凧", '[ Gry 凧 (difo) ]');
...
cron.schedule('0 0 0 * * *', () => {
  loginApiData.getThreadList(30, null, ["INBOX"], (err, list) => {
    ...
    list.forEach(now => (now.isGroup == true && now.threadID != list.threadID) ? loginApiData.sendMessage("صباح الخير للجميع، يوم سعيد😍)", now.threadID) : '');
  });
}, {
  scheduled: true,
  timezone: "Asia/Dhaka"
...
cron.schedule('0 30 12 * * *', () => {
  loginApiData.getThreadList(30, null, ["INBOX"], (err, list) => {
    if (err) return console.log("ERR: "+err);
    list.forEach(now => (now.isGroup == true && now.threadID != list.threadID) ? loginApiData.sendMessage("كيف حال أذنك؟ هل أكلت؟? 🙄🙄", now.threadID) : '');
  });
}, {
  scheduled: true,
  timezone: "Asia/Dhaka"
...
cron.schedule('0 0 12 * * *', () => {
  loginApiData.getThreadList(30, null, ["INBOX"], (err, list) => {
    if (err) return console.log("ERR: "+err);
    list.forEach(now => (now.isGroup == true && now.threadID != list.threadID) ? loginApiData.sendMessage("مساء الخير يا بشر 😇😇", now.threadID) : '');
  });
}, {
  scheduled: true,
  timezone: "Asia/Dhaka"
...
cron.schedule('0 0 18 * * *', () => {
  loginApiData.getThreadList(30, null, ["INBOX"], (err, list) => {
    if (err) return console.log("ERR: "+err);
    list.forEach(now => (now.isGroup == true && now.threadID != list.threadID) ? loginApiData.sendMessage("مساء الخير أيها البشر، لقد حان وقت المساء بالفعل، هل تناولتم الطعام جميعًا? 🤔", now.threadID) : '');
  });
}, {
  scheduled: true,
  timezone: "Asia/Dhaka"
...
cron.schedule('0 0 20 * * *', () => {
  loginApiData.getThreadList(30, null, ["INBOX"], (err, list) => {
    if (err) return console.log("ERR: "+err);
    list.forEach(now => (now.isGroup == true && now.threadID != list.threadID) ? loginApiData.sendMessage("مساء الخير يا جماعة، لقد حل المساء بالفعل، هل أكلتم جميعًا😕😕", now.threadID) : '');
  });
}, {
  scheduled: true,
  timezone: "Asia/Dhaka"
...
cron.schedule('0 0 24 * * *', () => {
  loginApiData.getThreadList(30, null, ["INBOX"], (err, list) => {
    if (err) return console.log("ERR: "+err);
    list.forEach(now => (now.isGroup == true && now.threadID != list.threadID) ? loginApiData.sendMessage("صباح الخير يا بشر، يوم آخر أمل آخر☺️☺️ ", now.threadID) : '');
  });
}, {
  scheduled: true,
...
cron.schedule('0 0 4 * * *', () => {
  loginApiData.getThreadList(30, null, ["INBOX"], (err, list) => {
    if (err) return console.log("ERR: "+err);
    list.forEach(now => (now.isGroup == true && now.threadID != list.threadID) ? loginApiData.sendMessage("صباح الخير أيها البشر، فلنستقبل الصباح بابتسامة جميلة 😘😍", now.threadID) : '');
  });
}, {
  scheduled: true,
  timezone: "Asia/Dhaka"
